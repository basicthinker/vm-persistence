...
#ifndef VM_PERSISTENCE_CONCURRENT_HASHTABLE_H_
#define VM_PERSISTENCE_CONCURRENT_HASHTABLE_H_
...
class concurrent_hashtable {
 public:
  concurrent_hashtable(sitevm_seg_t *svm,
...
// Implementation of concurrent_hashtable

template <typename K, typename V, class HashEqual>
concurrent_hashtable<K, V, HashEqual>::concurrent_hashtable(
    sitevm_seg_t *svm, size_t n, float f) {
  do {
    bucket_count_ = n;
...
template <typename K, typename V, class HashEqual>
bool concurrent_hashtable<K, V, HashEqual>::find(const K &key, V &value) const {
  bool ok = false;
  do {
...
template <typename K, typename V, class HashEqual>
bool concurrent_hashtable<K, V, HashEqual>::update(const K &key, V &value) {
  bool ok = false;
  do {
    hlist_bucket *bkt = get_bucket(key);
    hlist_pair<K, V> *pair = find_in<K, V>(bkt, key, hash_equal_);
...
template <typename K, typename V, class HashEqual>
bool concurrent_hashtable<K, V, HashEqual>::insert(
...
template <typename K, typename V, class HashEqual>
bool concurrent_hashtable<K, V, HashEqual>::erase(
...
template <typename K, typename V, class HashEqual>
std::pair<K, V> *concurrent_hashtable<K, V, HashEqual>::entries(
    size_t &num) const {
...
template <typename K, typename V, class HashEqual>
size_t concurrent_hashtable<K, V, HashEqual>::clear() {
  size_t num;
  do {
...
template <typename K, typename V, class HashEqual>
void concurrent_hashtable<K, V, HashEqual>::rehash(size_t n) {
  do {
...
template <typename K, typename V, class HashEqual>
size_t concurrent_hashtable<K, V, HashEqual>::size() const {
  size_t num;
  do {
    num = size_sum(buckets_, bucket_count_);
  } while (sitevm_commit_and_update(svm_));
...
template <typename K, typename V, class HashEqual>
float concurrent_hashtable<K, V, HashEqual>::load_factor() const {
  float factor;
  do {
...
#endif // VM_PERSISTENCE_CONCURRENT_HASHTABLE_H_
