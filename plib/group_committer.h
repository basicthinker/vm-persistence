...
class GroupCommitter : public VersionedPersistence<DataEntry> {
 ...
 private:
  ...
  static void Flush(GroupBuffer *buffer, sem_t *flush_sem_, bool *running);
};
...
inline GroupCommitter<DataEntry>::GroupCommitter(
  ...
  for (int i = 0; i < nlanes; ++i) {
    flushers_.push_back(std::thread(Flush, &buffer_, &flush_sem_, &running_));
  }
}
...
void GroupCommitter<DataEntry>::Flush(
    GroupBuffer *buffer, sem_t *flush_sem, bool *running) {
